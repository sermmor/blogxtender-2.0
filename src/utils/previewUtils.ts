...
/**
 * Builds the HTML notes zone section.
 */
export function createZonaNotas(
  ...
  const fondoStyle =
    colorNota ? `style="background-color: #${colorNota};"` : '';
...
/**
 * Builds the full exportable HTML entry from the textarea content.
 */
export function createCodeEntry(
  ...
  const [textoSinNotas, contentNotas] = convertirLlavesEnNotas(
    withBr,
    tituloNota,
    pasarANotas
  );
