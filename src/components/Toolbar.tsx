...
const Toolbar: React.FC<Props> = ({ activeTool, onToolToggle, mode }) => {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (showSpecial && specialRef.current && !specialRef.current.contains(e.target as Node)) {
  ...
  return (
    <div style={S.toolbarBar()}>
      ...
      <div ref={specialRef} style={{ position: 'relative' }}>
        <button
          style={S.toolbarBtn(showSpecial)}
      ...
      <button style={S.toolbarBtn()} title="UPPERCASE selection"
        onClick={() => substituteSelectionWithText(extractSelectedText().toUpperCase())}>
        Aᴬ
      </button>
      <button style={S.toolbarBtn()} title="lowercase selection"
        onClick={() => substituteSelectionWithText(extractSelectedText().toLowerCase())}>
        aᵃ
      </button>
      ...
      <button style={{ ...S.toolbarBtn(), fontWeight: 'bold' }} title="Bold (Ctrl+B)"
        onClick={() => ic('<span style="font-weight: bold;">', '</span>')}>
        B
      </button>
      <button style={{ ...S.toolbarBtn(), fontStyle: 'italic' }} title="Italic (Ctrl+I)"
        onClick={() => ic('<span style="font-style: oblique;">', '</span>')}>
        I
      </button>
      <button style={{ ...S.toolbarBtn(), textDecoration: 'underline' }} title="Underline"
        onClick={() => ic('<span style="text-decoration: underline;">', '</span>')}>
        U
      </button>
      <button style={{ ...S.toolbarBtn(), textDecoration: 'overline' }} title="Overline"
        onClick={() => ic('<span style="text-decoration: overline;">', '</span>')}>
        Ō
      </button>
      <button style={{ ...S.toolbarBtn(), textDecoration: 'line-through' }} title="Strikethrough"
        onClick={() => ic('<span style="text-decoration: line-through;">', '</span>')}>
        S̶
      </button>
      <button style={S.toolbarBtn()} title="Superscript"
        onClick={() => ic('<span style="vertical-align: super; font-size: 0.75em;">', '</span>')}>
        x²
      </button>
      <button style={S.toolbarBtn()} title="Subscript"
        onClick={() => ic('<span style="vertical-align: sub; font-size: 0.75em;">', '</span>')}>
        x₂
      </button>
      ...
      {/* Indent control + paragraph / alignment buttons */}
      <span style={S.toolbarLabel()}>⇥</span>
      <input
        type="number"
        min="0"
        style={S.toolbarIndentInput()}
        value={indent}
        onChange={(e) => setIndent(e.target.value)}
        title="Indentation in pt"
      />
      ...
      {/* Lists */}
      <button style={S.toolbarBtn()} title="List item"
        onClick={() => ic('<li>', '</li>')}>•</button>
      <button style={S.toolbarBtn()} title="Unordered list"
        onClick={() => ic('<ul>\n    <li>', '</li>\n</ul>')}>ul</button>
      <button style={S.toolbarBtn()} title="Ordered list"
        onClick={() => ic('<ol>\n    <li>', '</li>\n</ol>')}>ol</button>
      ...
      <div ref={linkRef} style={{ position: 'relative' }}>
        ...
        {showLink && (
          <LinkPopover
            onInsert={(url) => ic(`<a href="${url}" target='_blank'>`, '</a>')}
            onClose={() => setShowLink(false)}
          />
        )}
      </div>

      {/* Larger divider before sidebar tools */}
      ...
      {SIDEBAR_TOOLS.map((t) => (
        <button
          key={t.id}
          style={S.toolbarBtn(activeTool === t.id)}
          title={t.title}
